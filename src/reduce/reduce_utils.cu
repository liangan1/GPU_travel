...
template <typename T, int width = 32> __device__ void warp_reduce(T &val) {
...
__device__ void block_reduce(T &val) {
  ...
  static_assert(max_block_size <= 1024,
                "The number of thread in a block should less than 1024 to use "
                "the block_reduce");
...
template <typename T> __global__ void sum_cuda(T *array, int len) {
...
template <typename T> T sum(T *array, int len) {
  T sum = 0;
  for (int i = 0; i < len; i++) {
    sum += array[i];
  }
